...
function displayPics(){
  ...
  // It has to do with the scope of the variable. With var, the variable's scope is local to the entire function, so the console log has access to it because they are both contained within the function displayPics. With let, the variable's scope is local to the code block within which it is contained. Because the variable is declared with let inside a while loop, the variable only exists within that while block. Because the console log is outside of the while loop, it doesn't have access to that variable.
  console.log(viewed)
